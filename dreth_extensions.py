...
Two paths exposed:
  ...
  derive_compressions(agent, var, cycle) → List[Compression]
      Algebraic full-collapse: detects narrow operator + parent-fit
...
def derive_compressions(agent, var: int, cycle: int):
    """Detect operator+parent combinations that algebraically reduce to a
    ...
    NOT handled (would need richer Compression representation):
      PROD with one constant — yields scaled other parent, not a constant
      MEAN with one constant — yields linear in other parent
      MAX with non-extreme constant — depends on other parent
    ...
    try:
        ...
        # Helper: is parent p effectively a constant under its current fit?
        # Returns (is_const, value, tolerance) where tolerance is p's envelope ε.
        def parent_constant_value(p_idx):
            pn = agent.ledger.vars[p_idx]
            tol = pn.current_tolerance
            # Direct LOW/HIGH fit
            if pn.func == "LOW" and not pn.parents:
                return True, 0.2, tol
            if pn.func == "HIGH" and not pn.parents:
                return True, 0.8, tol
            # FIRST passthrough where the source itself is constant
            if pn.func == "FIRST" and pn.parents:
                source = pn.parents[0]
                if source < agent.world.visible_count:
                    return parent_constant_value(source)
            # Has a derived compression that's itself a constant + the parent
        ...
        if func == "FIRST" and parents:
            # FIRST(p, ...) ignores everything after first parent.
            # If p is constant, FIRST(p) = that constant.
            p = parents[0]
            is_const, p_val, p_tol = parent_constant_value(p)
            if is_const:
                gate = ((p, p_val, p_tol),)
        ...
        elif func == "DIFF" and len(parents) == 2:
            # DIFF(p, q) = |p - q|. If both are equal constants, DIFF = 0.
            p, q = parents
            p_const, p_val, p_tol = parent_constant_value(p)
            q_const, q_val, q_tol = parent_constant_value(q)
        ...
        elif func == "MEAN" and len(parents) == 2:
        ...
        elif func == "MAX" and len(parents) == 2:
            ...
            for hi_idx, lo_idx in [(0, 1), (1, 0)]:
                hi = parents[hi_idx]
                lo = parents[lo_idx]
                hi_const, hi_val, hi_tol = parent_constant_value(hi)
                if hi_const and hi_val >= 0.8 - hi_tol:
                    lo_const, lo_val, _ = parent_constant_value(lo)
        ...
        elif func == "MIN" and len(parents) == 2:
            # MIN(p, q): if p is fit-LOW (0.2) AND q's value never falls below it.
            for lo_idx, other_idx in [(0, 1), (1, 0)]:
                lo = parents[lo_idx]
                other = parents[other_idx]
                lo_const, lo_val, lo_tol = parent_constant_value(lo)
                if lo_const and lo_val <= 0.2 + lo_tol:
                    other_const, other_val, _ = parent_constant_value(other)
        ...
        elif func == "PROD" and len(parents) == 2:
...
def derive_equivalence_compressions(agent, var: int, cycle: int):
    ...
    constant under perturbation of the current state within each parent's
    ...
    try:
        ...
        for tie_set, count in agent.tie_log[var].items():
            ...
            # Build candidate gate from current parent values.
            gate_parts = []
            for p in n.parents:
            ...
            anchor_preds = []
            valid = True
            for hyp_parents, hyp_func in tie_set:
                if hyp_func not in FUNC_LIBRARY:
                    valid = False
                    break
                try:
                    par_vals = [anchor_state[p] for p in hyp_parents]
            ...
            # Validity check #2: perturb each parent within its envelope ε
